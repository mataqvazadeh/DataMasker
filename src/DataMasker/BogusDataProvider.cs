...
namespace DataMasker
{
    ...
    public class BogusDataProvider : IDataProvider
    {
            ...
            Name.Gender? gender)
        {
            ...
            {
                return GetValueInternal(columnConfig, obj, gender);
            }
            ...
            object newValue = GetValueInternal(columnConfig, obj, gender);
            ...
            Name.Gender? gender = null)
        {
            switch (columnConfig.Type)
            {
                ...
                case DataType.Bogus:
                    return _faker.Parse(PrepareStringFormat(columnConfig.StringFormatPattern, obj));
        ...
        // Its performance is awful, I know.
        // ToDo: Handle @ itself
        private static string PrepareStringFormat(string stringFormatPattern, IDictionary<string, object> obj)
        {
            if (!stringFormatPattern.Contains('@'))
            {
                return stringFormatPattern;
            }

            foreach (var keyValuePair in obj)
            {
                stringFormatPattern = stringFormatPattern.Replace($"@{keyValuePair.Key}", keyValuePair.Value?.ToString());
            }
